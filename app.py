...
@st.cache
def loadData():
    s = pd.read_csv('Josh_s.csv', index_col = 0); s.index = pd.to_datetime(s.index).dt.date
    p = pd.read_csv('Josh_p.csv', index_col = 0); p.index = pd.to_datetime(p.index).dt.date
